...
NL_SW_LTE_GELS3::NL_SW_LTE_GELS3(){
...
bool NL_SW_LTE_GELS3::Open(const std::string &device, int baudrate)
{
	return modem != nullptr && modem->Open(device, baudrate); 
}

bool NL_SW_LTE_GELS3::waitForReady(time_t timeout)
{
...
bool NL_SW_LTE_GELS3::ohShit(int fail_state)
{
	std::string response;
	while(fail_state != 7){
...
void  NL_SW_LTE_GELS3::set_error (void)
{
...
modem_device_t NL_SW_LTE_GELS3::Device()
{
...
std::string NL_SW_LTE_GELS3::Phone ()
{
	std::string result;
	std::string response;
	if(modem == nullptr){
...
std::string NL_SW_LTE_GELS3::IMEI ()
{
	std::string result;
	std::string response;
...
bool NL_SW_LTE_GELS3::getInfo(modem_info_t &info)
{
	//bool result = true;
	std::string response;
...
bool NL_SW_LTE_GELS3::PowerOn ()
{
	digitalWrite(PIN_MODEM_ON, HIGH);
	return true;
}

bool NL_SW_LTE_GELS3::PowerOff ()
{
	digitalWrite(PIN_MODEM_ON, LOW);	
	return true;
}

bool NL_SW_LTE_GELS3::hardReset()
{
#if defined DEBUG_MODEM
...
bool  NL_SW_LTE_GELS3::Reset () // This reset routine is not compliant with 4.3.2.0 firmware - Anthony
{
	if(modem == nullptr){
		throw(FailureException("Modem Hardware Error", "Modem is not initialized", "device_unavailable"));
	}
...
bool NL_SW_LTE_GELS3::sendRequest(const std::string &headers, modem_reply_t &message)
{
	std::string response;
	fail1 = fail2 = fail3 = fail4 = fail5 = 0;
	while(true){
